odoo.define('pos_retail.synchronization', function (require) {
    ...
    models.PosModel = models.PosModel.extend({
        ...
        push_message_to_other_sessions: function (value) {
            if (this.the_first_load || this.the_first_load == undefined || !this.config.bus_id) { // when cashier come back pos screen (reload browse) no need sync data
                return;
            }
            var self = this;
            if (!value['order_uid']) {
            ...
            return sending().fail(function (error, e) {
                ...
                if (error.message == "XmlHttpRequestError ") {
                    self.db.add_datas_false(message);
                }
            }).done(function () {
                self.trigger('reload:kitchen_screen');
            })
        },
        load_server_data: function () {
            ...
            return _super_posmodel.load_server_data.apply(this, arguments).then(function () {
                if (self.config.sync_multi_session && self.config.bus_id) {
                    self.sync_sessions = true;
                    self.chrome.call('bus_service', 'onNotification', self, function (notifications) {
                        _.each(notifications, (function (notification) {
                            if (notification[0][1] === 'pos.bus') {
                                console.log('sync between session coming');
                                var data_sync = JSON.parse(notification[1]);
                                self.syncing_sessions(data_sync['value']);
                            }
                        }).bind(self));
                    });
                    ...
                    console.log('sync between session not active')
                }
            })
        },
        repush_to_another_sessions: function () {
            ...
            var datas_false = this.db.get_datas_false();
            if (datas_false && datas_false.length) {
                ...
                sending().fail(function () {
                    ...
                    for (var i = 0; i < datas_false.length; i++) {
                        self.db.remove_data_false(datas_false[i]['sequence']);
                    }
                    self.gui.show_popup('notify_popup', {
                        title: 'ALERT',
                        from: 'top',
    ...
    models.Order = models.Order.extend({
            ...
            if (this.pos.sync_sessions) {
                this.bind('add', function (order) { // syncing New order
                    if ((self.pos.the_first_load == false || !self.pos.the_first_load) && (order.syncing != true || !order.syncing) && (order.temporary == false || !order.temporary) && self.pos.config.bus_id && self.pos.config.bus_id[0]) {
                        self.pos.push_message_to_other_sessions({
        ...
        finalize: function () {
            var self = this;
            if (this.pos.sync_sessions && (this.syncing != true || !this.syncing)) {
        ...
        set_client: function (client) {
            ...
            if (client && client['property_product_pricelist']) {
                ...
                if ([11, 12].indexOf(this.pos.server_version) != -1 && pricelist && !this.is_return) { // we're don't want set price list for order return
            ...
            if ((!this.syncing || this.syncing == false) && this.pos.sync_sessions && this.pos.the_first_load == false) {
                var order = this.export_as_JSON()
                if (client) {
                    this.pos.push_message_to_other_sessions({
                        data: {
                            uid: order['uid'],
        ...
        set_pricelist: function (pricelist) { // only v11 have
            ...
            if (this.pos.sync_sessions && (this.syncing != true || !this.syncing) && this.pos.the_first_load == false) {
                var order = this.export_as_JSON();
                if (!order['uid']) {
                    return;
                }
    ...
    models.Orderline = models.Orderline.extend({
        ...
        merge: function (orderline) {
            ...
            if ((orderline.syncing == false || !orderline.syncing) && this.pos.sync_sessions) {
        ...
        trigger_update_line: function () {
            ...
            if (this.pos.sync_sessions && (!this.syncing || this.syncing == false) && (this.order.syncing == false || !this.order.syncing) && (this.uid && this.order.temporary == false)) {
