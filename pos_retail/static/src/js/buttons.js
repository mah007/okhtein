odoo.define('pos_retail.buttons', function (require) {
                ...
                return this.gui.show_popup('confirm', {
                    ...
                    confirm: function () {
                        ...
                        if (order) {
                            order['lock'] = true;
                            self.pos.lock_order();
                            if (self.pos.sync_sessions) { // v12 dont merge
                                self.pos.push_message_to_other_sessions({ // v12 dont merge
                                    data: order.uid,
                                    action: 'lock_order',
    ...
    var button_lock_unlock_order = screens.ActionButtonWidget.extend({
        ...
        button_click: function () {
            ...
            order.trigger('change', order);
            if (this.pos.sync_sessions) {  // v12 dont merge
                var action;
                if (order['lock']) {
                    action = 'lock_order';
                } else {
                ...
                this.pos.push_message_to_other_sessions({  // v12 dont merge
                    data: order.uid,
                    action: action,
