odoo.define('pos_retail.pos_chanel', function (require) {
    ...
    models.PosModel = models.PosModel.extend({
        sync_with_backend: function (data) {
            ...
            var old_caches = this.database[model];
            var new_caches = _.filter(old_caches, function (cache) {
                return cache['id'] != data['id'];
            });
            new_caches.push(data);
            this.database[model] = new_caches;
            if (model == 'product.product') {
                this.trigger('sync:product', data);
            }
            if (model == 'res.partner') {
                this.trigger('sync:partner', data);
            }
            if (model == 'product.pricelist' && this.config.sync_pricelist == true) {
                this.syncing_pricelist(data)
            }
            if (model == 'product.pricelist.item' && this.config.sync_pricelist == true) {
                this.syncing_pricelist_item(data)
            }
            if (model == 'account.invoice' && this.config.management_invoice) {
                this.db.save_data_sync_invoice(data);
                this.trigger('update:invoice');
            }
            if (model == 'account.invoice.line' && this.config.management_invoice) {
                this.db.save_data_sync_invoice_line(data);
                this.trigger('update:invoice');
            }
            if (model == 'pos.order' && this.config.pos_orders_management) {
                this.db.save_data_sync_order(data);
                this.trigger('update:order');
            }
            if (model == 'pos.order.line' && this.config.pos_orders_management) {
                this.db.save_data_sync_order_line(data);
            }
            if (model == 'sale.order' && this.config.sync_sale_order == true) {
                this.db.sync_sale_order(data);
                this.trigger('sync:sale_orders', data['id']);
            }
            if (model == 'sale.order.line' && this.config.sync_sale_order == true) {
                this.db.sync_sale_order_lines(data);
                this.trigger('sync:sale_orders', data['id']);
            }
            ...
            return _super_posmodel.load_server_data.apply(this, arguments).then(function () {
                self.chrome.call('bus_service', 'onNotification', self, function (notifications) {
                    _.each(notifications, (function (notification) {
                        if (notification[0][1] === 'pos.sync.data') {
                            console.log('new notify from backend');
                            self.sync_with_backend(notification[1])
                        }
                    }).bind(self));
                });
            })
